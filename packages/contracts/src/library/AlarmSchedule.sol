...
library AlarmSchedule {
  ...
  function init(
    ...
    require(
      -43200 < timezoneOffset && timezoneOffset < 43200 && timezoneOffset % 1 hours == 0,
  ...
  function entries(Schedule storage self) internal view started(self) returns (uint) {
  ...
  function recordEntry(Schedule storage self) internal started(self) {
    ...
    require(timeSinceLastEntry >= 1 days - self.submissionWindow, "ALREADY_SUBMITTED_TODAY");
    require(inSubmissionWindow(self), "NOT_IN_SUBMISSION_WINDOW");
    self.lastEntryTime = block.timestamp;
    self.alarmEntries++;
  }

  function inSubmissionWindow(Schedule storage self) internal view started(self) returns (bool) {
    if (_deadlinePassedToday(self)) {
      return false;
    }
    return (_nextDeadlineInterval(self) - block.timestamp) < self.submissionWindow;
  }
   ...
   */
  function missedDeadlines(Schedule storage self) internal view started(self) returns (uint numMissedDeadlines) {
    ...
    uint8 startDayOfWeek = _dayOfWeek(_offsetTimestamp(lastDeadline - (remainderDays * 1 days), self.timezoneOffset));
    uint8 endDayOfWeek = _dayOfWeek(_offsetTimestamp(lastDeadline, self.timezoneOffset));

    uint8 alarmsInRemainderDays = 0;
    for (uint j = 0; j < self.alarmDays.length; j++) {
      // I'm sorry you're reading this.
      if (
        (self.alarmDays[j] >= startDayOfWeek && self.alarmDays[j] <= endDayOfWeek) ||
        (startDayOfWeek > endDayOfWeek && (self.alarmDays[j] >= startDayOfWeek || self.alarmDays[j] <= endDayOfWeek))
      ) {
  ...
  function timeToNextDeadline(Schedule storage self) internal view started(self) returns (uint) {
    return nextDeadlineTimestamp(self) - block.timestamp;
  }

  function nextDeadlineTimestamp(Schedule storage self) internal view started(self) returns (uint) {
    uint referenceTimestamp = _lastDeadlineInterval(self);

    uint8 curDay = _dayOfWeek(_offsetTimestamp(referenceTimestamp, self.timezoneOffset));
  ...
  function _nextAlarmDay(Schedule storage self, uint8 currentDay) internal view returns (uint8) {
    /**
  ...
  function _nextDeadlineInterval(Schedule storage self) internal view returns (uint256) {
  ...
  function _lastDeadlineInterval(Schedule storage self) internal view returns (uint256) {
    uint lastMidnight = _lastMidnightTimestamp(self);
    if (_deadlinePassedToday(self)) {
      return lastMidnight + self.alarmTime;
    } else {
  ...
  function _deadlinePassedToday(Schedule storage self) internal view returns (bool) {
   ...
   */
  function _lastMidnightTimestamp(Schedule storage self) internal view returns (uint) {
    uint localTimestamp = _offsetTimestamp(block.timestamp, self.timezoneOffset);
    uint lastMidnightLocal = localTimestamp - (localTimestamp % 1 days);
    return _offsetTimestamp(lastMidnightLocal, -self.timezoneOffset);
  }

  function _offsetTimestamp(uint timestamp, int offset) internal pure returns (uint256) {
